...
- ~~fetch_og_tags (URL-preview webview tier) is dead in prod~~ — RESOLVED (2026-08-27). Root cause was broader than "a missing entry": the production window loads the sidecar origin, which tauri's ACL treats as **remote**, and remote origins have *every* app-defined command denied unless a capability names it (see the check in tauri webview/mod.rs). It applied to `restart_for_update` too, which is why the auto-update "Restart" button did nothing in 0.3.1 while working in dev, where the origin is local. Fixed by declaring both commands in build.rs (generating allow-fetch-og-tags / allow-restart-for-update) and granting them in capabilities/default.json.
