...
import { BACKEND_CREDENTIALS, BACKEND_CROSS_ORIGIN, BACKEND_URL } from "./backend.ts"
...
export default class SSEClient extends RPCClient {
	...
	#pingLoop = () => {
		...
		fetch(`${BACKEND_URL}_gomuks/sse/ping?${params}`, {
			method: "POST",
	...
	request<Req, Resp>(command: string, data: Req): CancellablePromise<Resp> {
		...
		return new CancellablePromise((resolve, reject) => {
			fetch(`${BACKEND_URL}_gomuks/exec/${command}`, {
				method: "POST",
				credentials: BACKEND_CREDENTIALS,
